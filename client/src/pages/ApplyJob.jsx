  ...
  const applyHander = async () => {
    try {
      ...
      if (data.success) {
        ...
        setIsAlreadyApplied(true) // ✅ mark as applied
      } else {
        toast.error(data.message)
      }
    } catch (error) {
          ...
          <div className='flex justify-center md:justify-between flex-wrap gap-8 px-14 py-20 mb-6 bg-sky-50 border border-sky-400 rounded-xl'>
            ...
            <div className='flex flex-col justify-center text-sm max-md:mx-auto max-md:text-center'>
              <button
                ...
                className={`p-2.5 px-10 rounded ${isAlreadyApplied ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white'}`}
              >
          ...
          <div className='flex flex-col lg:flex-row justify-between items-start'>
            ...
            <div className='w-full lg:w-2/3'>
              ...
              <button
                onClick={applyHander}
                disabled={isAlreadyApplied}
                className={`p-2.5 px-10 rounded mt-10 ${isAlreadyApplied ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 text-white'}`}
              >
                {isAlreadyApplied ? 'Already Applied' : 'Apply Now'}
              </button>
            </div>
